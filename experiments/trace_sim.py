...
def run(args, report_db, validationDataset):
    ...
    # bug_db, df_coef, pos_coef, diff_coef, match_cost, gap_penalty, mismatch_penalty, max_depth, nthreads, filter_recursion=0, df_threshold=0.0, field_coef=0.0

    arguments = [args.df_coef, args.pos_coef, args.diff_coef, args.match_cost, args.gap_penalty, args.mismatch_penalty,
                 args.sigmoid, args.gamma, args.sum, args.idf, args.const_match, args.reciprocal_func, args.no_norm, args.const_gap, args.const_mismatch, args.brodie_function]
    # bug_db, method, agg_strategy, arguments, max_depth, nthreads, filter_recursion, df_threshold, field_coef, keep_ukn, static_df_ukn
    scorer = Scorer(report_db, SimilarityMethod.TRACE_SIM, args.aggregate, arguments, args.max_depth,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-df_coef', default=1.0, type=float, help="")
    parser.add_argument('-pos_coef', default=1.0, type=float, help="")
    parser.add_argument('-diff_coef', default=1.0, type=float, help="")

    parser.add_argument('-match_cost', default=1.0, type=float, help="")
    parser.add_argument('-gap_penalty', default=1.0, type=float, help="")
    parser.add_argument('-mismatch_penalty', default=1.0, type=float, help="")

    parser.add_argument('-sigmoid', action="store_true", help="")
    parser.add_argument('-gamma', default=1.0, type=float, help="")

    parser.add_argument('-idf', action="store_true", help="")
    parser.add_argument('-sum', action="store_true", help="")

    parser.add_argument('-const_match', action="store_true", help="")
    parser.add_argument('-reciprocal_func', action="store_true", help="")
    parser.add_argument('-no_norm', action="store_true", help="")

    parser.add_argument('-const_gap', action="store_true", help="")
    parser.add_argument('-const_mismatch', action="store_true", help="")
    parser.add_argument('-brodie_function', action="store_true", help="")
